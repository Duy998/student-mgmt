...
def download_question_template(_: User = Depends(get_admin_user_or_apikey)):
    ...
    headers = ["Code", "Grade", "Topic", "Level", "Type", "Question", "A", "B", "C", "D", "Answer"]
    ...
    ws.append(["L6-001", 6, "Information and Data", "Recall", "MCQ",
               "Question 1. Knowledge content about Information and Data.",
               "Option A", "Option B", "Option C", "Option D", "A"])
    ws.append(["L6-002", 6, "Computer Devices", "Recall", "MCQ",
               "Question 2. Knowledge content about Computer Devices.",
...
async def import_questions(
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        if not code:
            ...
            row_errors.append(f"Code is duplicated with row {codes_seen[code]}")
        else:
            codes_seen[code] = idx
        ...
        if not topic:
            row_errors.append("Missing Topic")
        if not level:
            row_errors.append("Missing Level")
        if not content_text:
            row_errors.append("Missing Question")
        if not a or not b or not c or not d:
            row_errors.append("Missing one or more options A/B/C/D")
        if answer.upper() not in VALID_ANSWERS:
            row_errors.append(f"Answer '{answer}' is invalid (must be A/B/C/D)")
    ...
    if errors:
        raise HTTPException(status_code=422, detail={
            "message": f"Import failed: {len(errors)} errors. No questions were saved.",
            "errors": errors,
        })
...
def submit_quiz(
    ...
    if not payload.answers:
        raise HTTPException(status_code=400, detail="No answers available.")
